...
reg_get() {
    # Values are stored as KEY="VALUE"  # cat:...
...
# Update the env registry with confirmed Termux binary locations.
# Defined early because it is called before and after install.
...
# ── network check ─────────────────────────────────────────────
# Probes connectivity using ping (available in Busybox/Android) or nc.
# Uses Google's public DNS IP by default; override HOM_NETWORK_PROBE_IP
# in the env registry before this script runs for restrictive networks.

wait_for_network() {
    local max_wait=120
    local elapsed=0
    local probe_ip
    probe_ip=$(grep "^HOM_NETWORK_PROBE_IP=" "$ENV_REGISTRY" 2>/dev/null | \
               cut -d= -f2- | cut -d' ' -f1)
    probe_ip="${probe_ip:-8.8.8.8}"
    log "Waiting for network connectivity via $probe_ip (max ${max_wait}s)..."
    while [ "$elapsed" -lt "$max_wait" ]; do
        if ping -c 1 -W 2 "$probe_ip" >/dev/null 2>&1 ||
           (echo "" | nc -w 2 "$probe_ip" 53) >/dev/null 2>&1; then
...
# Install each package individually so a single failure doesn't abort the rest.
for pkg in $REQUIRED_PACKAGES; do
    if command -v run-as >/dev/null 2>&1; then
        run-as com.termux "$TERMUX_PREFIX/bin/pkg" install -y "$pkg" >> "$LOG" 2>&1 || \
            log "  pkg install $pkg returned non-zero (check $LOG)"
    else
        "$TERMUX_PREFIX/bin/pkg" install -y "$pkg" >> "$LOG" 2>&1 || \
            log "  pkg install $pkg returned non-zero (check $LOG)"
    fi
done
